...
const SourceVideosBucket = process.env.SourceVideosBucket;
...
exports.handler = function(event, context) {
    ...
    if (event.RequestType == "Create") {
        ...
        let jobs = Promise.all([uploadUIAssets(event),uploadVideoAssets(event)]);
...
function uploadVideoAssets(event) {
    ...
    var lineReader = readline.createInterface({
        input: S3.getObject({
            Bucket: SourceVideosBucket,
            Key: SourceVideoAssetsPrefix + '/video-manifest.txt'
        }).createReadStream()
    });
    ...
    lineReader.on('line', function (line) {
        ...
        var params = {
            Bucket: SourceBucket,
            CopySource: SourceVideosBucket + '/' + line,
